...
function itemsTable(itens) {
  ...
  const totalGeral = itens.reduce((s,i) => s + (parseFloat(i.preco_unitario) * i.quantidade), 0);
  ...
  return `
    <table width="100%" cellpadding="0" cellspacing="0" style="margin:20px 0;">
      ...
      <tr>
        ...
        <td style="padding-top:12px;font-size:15px;font-weight:700;color:#c0143c;text-align:right;">${fmtBRL(totalGeral)}</td>
      </tr>
    </table>`;
}
...
async function sendPixEmail({ to, nome, pedidoId, total, itens, pixCode }) {
  ...
  const content = `
    ...
    <p style="margin:28px 0 0;font-size:13px;color:#888;">Dúvidas? Fale com a gente pelo WhatsApp ou e-mail.</p>`;
...
async function sendCardApprovedEmail({ to, nome, pedidoId, total, itens }) {
  ...
  const content = `
    ...
    <p style="margin:28px 0 0;font-size:13px;color:#888;">Obrigado por escolher a Beaver Books!</p>`;
...
async function sendCardPendingEmail({ to, nome, pedidoId, total, itens }) {
  ...
  const content = `
    ...
    <div style="background:#fefce8;border:1.5px solid #fde047;border-radius:8px;padding:16px;margin-bottom:24px;">
      ...
      <p style="margin:6px 0 0;font-size:13px;color:#854d0e;">Seu pagamento está sendo analisado. Você receberá outro e-mail assim que confirmado.</p>
    </div>

    <p style="margin:0 0 8px;font-size:13px;font-weight:600;color:#555;text-transform:uppercase;letter-spacing:.5px;">Resumo do pedido</p>
    ${itemsTable(itens)}

    <p style="margin:28px 0 0;font-size:13px;color:#888;">Qualquer dúvida estamos à disposição!</p>`;
